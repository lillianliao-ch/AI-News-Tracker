...
| **Phase 2 V3** | **全量轻富化** | 🆕 待执行 | — | 对全部 6081 人做 Repo+网站探活+Scholar |
| **Phase 3 V3** | **统一 AI 判定** | 🆕 待执行 | — | 多维信号一次性筛选（bio+repo+scholar+语言） |
| Phase 3/3.5 | 深度富化 | ✅ 完成 (3167人) | 66.9% 邮箱 | 仅对 AI 人才执行网站爬取+Scholar |
| Phase 5 | 入库+统一评级打标 | ✅ 完成 (3167人) | S/A/B/C + V2 标签 | 新增 `tier_updated_at` 追踪重评 |
| Phase 6 | 分层触达 | ✅ 引擎就绪 | V2 Prompt 路由 | LinkedIn/邮件按标签智能选模板 |
| Phase 7 | 持续跟进 | ❌ 未开始 | — | CRM + Nurture |
...
| **2/15** | **Phase 3.5 大规模拓展** | **借助 Lamda Scraper 工具全量网站深挖，最终有效入库 3,167 人** |

**当前重点**: Phase 3.5 和 Phase 6.5 已全量完成，3,167 人完成富化并打标。当前核心资产：有邮箱的候选人达 2,120 人 (66.9%)。可选下一步：
1. **Phase 7 邮件触达** — 针对 P1(256人, 有LinkedIn) 和 P2(1,245人, AI活跃) 立即展开精准触达
2. **Phase 4 社交网络扩展** — 从现有高质量种子的 Following 中交叉挖掘全新候选人
...
2. **采集脚本位置**: `/Users/lillianliao/notion_rag/github_mining/scripts/github_network_miner.py`
3. **猎头系统**: `/Users/lillianliao/notion_rag/personal-ai-headhunter/`
4. **系统操作脚本**: `personal-ai-headhunter/import_github_candidates.py`, `scripts/label_github_candidates.py`, `scripts/fetch_github_bios.py`
...
## Runbook 1.5: Priority 标签 + Bio 筛选 (Phase 6.5) ⭐ 新增

> 对已入库的 GitHub 候选人进行 Priority 分级（P1/P2/P3）和 Bio 二次筛选
> **前置**: Phase 6 已完成入库
...
| **方案 B（V3 流程）** | Phase 1 → Phase 2 V3 → Phase 3 V3 → Phase 3.5 | 2-3 小时 | 85% | ⭐ **推荐** |

**V3 流程优势**:
- ✅ **更快**: 轻量级富化，API 请求减少 60%
- ✅ **更准**: 多维信号判定（bio + repo + scholar + 语言），减少误杀
- ✅ **更全**: 自动发现被旧流程误判的 AI 人才
...
- 预期产出: ~6,100 人基础资料
- 耗时: 1-2 小时
- 产出: `github_mining/scripts/github_mining/phase1_seed_users.json`
...
from database import SessionLocal, Candidate
session = SessionLocal()
gh = session.query(Candidate).filter(Candidate.source.ilike('%github%'), C.linkedin_url != None).all()
from collections import Counter
...
```bash
# 生成 LinkedIn 消息草稿
python3 personal-ai-headhunter/batch_linkedin_outreach.py --priority linkedin-first

# 生成邮件草稿 (仅无LinkedIn候选人)
python3 personal-ai-headhunter/batch_email_outreach.py --filter-no-linkedin

# GitHub 批量 Follow
python3 personal-ai-headhunter/batch_github_follow.py --limit 100
```
