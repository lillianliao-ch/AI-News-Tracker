...
| **import_github_candidates.py 未导入字段** | LLM 提取的 7 个字段未映射到数据库 | 添加 `extracted_work_history`, `extracted_education`, `extracted_skills`, `talking_points`, `nationality`, `nationality_confidence`, `website_quality_score` |

**影响**:
- Phase 4.5 现在可以正确执行并验证
- LLM 深度富化的数据会被正确导入数据库
- 候选人工作履历、教育背景、技能标签、国籍检测等 AI 提取数据可用
