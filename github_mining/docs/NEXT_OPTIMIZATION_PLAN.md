...
### 3. 高级断点管理系统 (Advanced Progress State)
现有的断点续传是基于记事本增量写入（`{"completed": []}`）。当人数膨胀到 10 万级，每次载入大数组会变慢。
- **行动**：将批次内部的进度管理从 JSON 文件升级为一个轻量级的本地 SQLite 表或类似 KV 库的机制（如 `DiskCache`）。
- **价值**：毫秒级判定“此人是否已处理”，极大地加速大规模重跑时的过滤速度。
