...
def main():
    cat_dir = os.path.join(BASE_RESEARCH_DIR, "Seed_Org_Chart")
    ...
    FROM candidates 
    WHERE (
        current_company LIKE '%字节%' OR 
        current_company LIKE '%ByteDance%' OR 
        company_normalized LIKE '%字节%' OR
        work_experiences LIKE '%字节%' OR
        work_experiences LIKE '%ByteDance%' OR
        raw_resume_text LIKE '%字节%'
    ) AND (
        work_experiences LIKE '%Seed%' OR
        project_experiences LIKE '%Seed%' OR
        current_title LIKE '%Seed%' OR
        raw_resume_text LIKE '%Seed%'
    )
    """
    cur.execute(query)
    rows = cur.fetchall()
    ...
    print(f"[{len(rows)}] Multi-faceted candidates mentioning ByteDance and Seed discovered.")
    if not rows:
        return
        
    chunk_size = 8  # Smaller chunk size because resume text is large
    chunks_created = 0
...
现在你收到了这批几十位（甚至上百位）来自“字节跳动 / ByteDance / Seed团队 以及其上下游”的候选人真实履历记录。

请根据所有这些履历中提及的**工作经历、职责边界、负责的具体项目代号、交叉协作方、甚至是偶尔提及的汇报人 (Reporting Line) 等草蛇灰线**，帮我：
1. **完整逆向拼凑还原出 字节跳动大模型 Seed 团队 的组织架构（Org Chart）**。
   用极具职业猎头直觉的方式，梳理出包含“大模型基础组”、“评测层”、“数据团队”、“基础设施多模态组”、“RLHF强化学习”等可能存在的内部划分支线。
2. **【核心】呈现一张层级树状图矩阵**（请使用 Markdown list）。
   请把你能识别出来的**核心成员（列出ID或对应的Title特征，能拼凑出多少是多少）**放在这棵树的正确位置上。
3. **技术攻坚方向窥探**：通过这些人最近半年/一年主要写的“项目经历”，分析出 Seed 团队当下重点押注的2-3个尚未公开或正在强攻的技术高地是什么？
4. 如果数据不够拼全，请大胆根据你对通用大厂大模型架构的常识进行**合理推演补齐**，并在你不确定的地方打上 `(?)` 问号标识。

请输出极其专业、震撼的报告！"""
             
    report_path = os.path.join(cat_dir, "Seed_Org_Chart_Mapping.md")
