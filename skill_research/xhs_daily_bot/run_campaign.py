...
def fetch_top5():
    ...
    clean = [{"id": i, "title": it.get("title", ""), "url": it.get("url", "")} for i, it in enumerate(items)]
    
    prompt = f"""
    挑选出最适合发在小红书引发程序员深度讨论的前 5 条关于 AI/大模型的科技新闻。
    列表：{json.dumps(clean, ensure_ascii=False)}
    要求：仅返回纯JSON数组：[{{ "title": "...", "reason": "...", "url": "..." }}]
    """
...
def main():
    print(f"🚀 [1/4] Daily Campaign Started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    try:
        top5 = fetch_top5()
    except Exception as e:
        print(f"❌ Failed to fetch/score news: {e}")
        send_telegram_msg(f"🚨 <b>XHS Bot Error</b>\\nSourcing phase failed: {e}")
        return

    print("🧠 [2/4] Generating 10 Drafts (A/B Styles)...")
    drafts = []
    for i, item in enumerate(top5, 1):
        for style, P in [("A", PROMPT_A), ("B", PROMPT_B)]:
            ...
            filepath = os.path.join(WORKSPACE_DIR, "drafts", f"topic_{i}_{style}.md")
            ...
            print(f"   ✓ Draft Topic {i} Style {style} ready.")
    ...
    for idx, d in enumerate(drafts, 1):
        ...
        img_path = os.path.join(WORKSPACE_DIR, "drafts", f"cover_{idx}.jpg")
        ...
        try:
            ...
            success_count += 1
            time.sleep(3) # Small buffer between UI automations
    ...
    result_text = (
        "🤖 <b>小红书运营助手播报</b>\\n\\n"
        f"✅ <b>今日打分完成</b> (选取 Top 5 科技新闻)\\n"
        f"✅ <b>撰稿完成</b> (严格遵循 A/B 两版人设)\\n"
        f"✅ <b>推流完成</b> (成功自动保存 {success_count}/10 篇草稿)\\n\\n"
        "🔗 <i>请打开手机小红书 App 挑选发布！</i>"
    )
    send_telegram_msg(result_text)
    print("🎉 All Pipeline Operations Flushed Successfully!")
