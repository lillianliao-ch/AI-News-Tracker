...
def render_prompt_config():
    ...
    user_job_prompts = db.query(SystemPrompt).filter_by(prompt_type='job', prompt_role='user').all()
    ...
    with tab1:
        ...
        for p in user_cand_prompts:
            with st.expander(f"模版: {p.prompt_name} {'(Active)' if p.is_active else ''}"):
                # 初始化编辑内容
                edit_key = f"edit_user_p_{p.id}"
                if edit_key not in st.session_state:
                    st.session_state[edit_key] = p.content
                
                # 创建显示的内容
                display_content = st.session_state[edit_key]
                edited_content = st.text_area("Content", value=display_content, height=200, key=edit_key)
                
                col1, col2, col3 = st.columns([1, 1, 2])
                with col1:
                    if st.button("💾 保存修改", key=f"save_p_{p.id}"):
                        p.content = edited_content
                        st.session_state[edit_key] = edited_content  # 更新session state
                        db.commit()
                        st.success(f"模版 '{p.prompt_name}' 已更新")
                        st.rerun()
                with col2:
                    if st.button("🔄 重置", key=f"reset_p_{p.id}"):
                        st.session_state[edit_key] = p.content
                        st.rerun()
                with col3:
                    if st.button("🗑️ 删除", key=f"del_p_{p.id}"):
                        db.delete(p)
                        db.commit()
                        st.rerun()
    ...
    with tab2:
        ...
        # List existing user prompts
        for p in user_job_prompts:
            with st.expander(f"模版: {p.prompt_name} {'(Active)' if p.is_active else ''}"):
                # 初始化编辑内容
                edit_key = f"edit_user_jp_{p.id}"
                if edit_key not in st.session_state:
                    st.session_state[edit_key] = p.content
                ...
                col1, col2, col3 = st.columns([1, 1, 2])
                with col1:
                    if st.button("💾 保存修改", key=f"save_jp_{p.id}"):
                        p.content = edited_content
                        st.session_state[edit_key] = edited_content  # 更新session state
                        db.commit()
                        st.success(f"模版 '{p.prompt_name}' 已更新")
                        st.rerun()
                with col2:
                    if st.button("🔄 重置", key=f"reset_jp_{p.id}"):
                        st.session_state[edit_key] = p.content
                        st.rerun()
                with col3:
                    if st.button("🗑️ 删除", key=f"del_jp_{p.id}"):
                        db.delete(p)
                        db.commit()
                        st.rerun()
        
        with st.expander("➕ 新增模版"):
            new_j_name = st.text_input("模版名称", key="new_job_p_name")
            new_j_content = st.text_area("模版内容 ({raw_text} 为占位符)", height=150, key="new_job_p_content")
            if st.button("保存模版", key="save_new_job_p"):
                _add_user_prompt(db, 'job', new_j_name, new_j_content)
                st.success("模版已添加")
                st.rerun()
