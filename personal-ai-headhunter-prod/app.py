    ...
    if st.session_state.view_mode == 'detail' and st.session_state.selected_candidate_id:
        ...
        with tab6:
            ...
            if mode == "⚡ 快速录入":
                ...
                st.info("💡 标准录入适用于完整的人才信息录入")

                # 初始化表单状态
                if 'std_exp_count' not in st.session_state:
                    st.session_state.std_exp_count = 0
                if 'std_edu_count' not in st.session_state:
                    st.session_state.std_edu_count = 0
                ...
                with st.form("standard_add_form"):
                    ...
                    preset_skills = {
                        ...
                        "AI/ML": ["机器学习", "深度学习", "NLP", "CV", "LLM", "Agent", "RAG"],
                    ...
                    custom_skills = st.text_input(
                        "🏷️ 自定义技能（逗号分隔）",
                        placeholder="例如: Kafka, Redis, Elasticsearch"
                    )
                    if custom_skills:
                    ...
                    st.markdown("### 工作经历")
                    if st.button("+ 添加工作经历"):
                        st.session_state.std_exp_count += 1
                        st.rerun()

                    work_experiences = []
                    for i in range(st.session_state.std_exp_count):
                        with st.expander(f"工作经历 #{i+1}", expanded=True):
                            col_w1, col_w2 = st.columns(2)
                            with col_w1:
                                exp_company = st.text_input("公司", key=f"exp_company_{i}")
                            with col_w2:
                                exp_title = st.text_input("职位", key=f"exp_title_{i}")
                            exp_time = st.text_input("时间", placeholder="2020-01 至 2023-12", key=f"exp_time_{i}")
                            exp_desc = st.text_area("工作内容", key=f"exp_desc_{i}", height=100)

                            if exp_company:  # 只保存有公司名称的经历
                                work_experiences.append({
                                    "company": exp_company,
                                    "title": exp_title,
                                    "time": exp_time,
                    ...
                    st.markdown("### 教育经历")
                    if st.button("+ 添加教育经历"):
                        st.session_state.std_edu_count += 1
                        st.rerun()

                    education_details = []
                    for i in range(st.session_state.std_edu_count):
                        with st.expander(f"教育经历 #{i+1}", expanded=True):
                            col_e1, col_e2, col_e3 = st.columns(3)
                            with col_e1:
                                edu_school = st.text_input("学校", key=f"edu_school_{i}")
                            with col_e2:
                                edu_major = st.text_input("专业", key=f"edu_major_{i}")
                            with col_e3:
                                edu_degree_options = ["本科", "硕士", "博士", "大专", "其他"]
                                edu_degree = st.selectbox("学历", edu_degree_options, key=f"edu_degree_{i}")
                            edu_year = st.text_input("时间", placeholder="2015-2019", key=f"edu_year_{i}")

                            if edu_school:  # 只保存有学校的经历
                                education_details.append({
                                    "school": edu_school,
                                    "major": edu_major,
                                    "degree": edu_degree,
                    ...
                    if submitted:
                        if not std_name or not std_company or not std_title:
                            ...
                            summary_parts = []
                            if selected_skills:
                                summary_parts.append(f"**技能标签**: {', '.join(selected_skills[:10])}")
                            if work_experiences:
                                summary_parts.append(f"**工作经历**: {len(work_experiences)}段")
                            if education_details:
                                summary_parts.append(f"**教育背景**: {education_details[0]['school'] if education_details else ''}")
                    ...
                    if uploaded_file.name.endswith('.txt'):
                        ...
                        if st.button("🚀 开始 AI 解析", type="primary"):
                            with st.spinner("正在解析简历..."):
                                # 获取活跃的 prompt 模板
                                active_prompts = db.query(SystemPrompt).filter_by(
                                ...
                                for p in active_prompts:
                                    if p.prompt_role == 'system':
                                        system_prompt = p.content
                                    elif p.prompt_role == 'user':
                                        user_prompt = p.content

                                # 调用 AI 解析
                                parsed_data = AIService.parse_resume(
                                    resume_content,
                                ...
                                if parsed_data and parsed_data.get("name") != "Parse Error":
                                    ...
                                    # 确认保存
                                    if st.button("💾 确认保存到人才库", type="primary"):
                                        new_candidate = Candidate(
                                            ...
                                            raw_resume_text=resume_content,
                                            source_file=f'upload_{uploaded_file.name}',
                                            created_at=datetime.now()
                                        )
                        ...
                        st.warning("⚠️ PDF 解析功能需要安装 PyPDF2: `pip install PyPDF2`")
                        try:
                            import PyPDF2
                        ...
                        st.warning("⚠️ 暂不支持此格式，请使用 TXT 或 PDF")
                else:
                    st.info("👆 请先上传简历文件")
