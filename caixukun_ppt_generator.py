...
def create_gradient_slide(slide, title, subtitle=None):
    """为幻灯片添加渐变色背景效果（通过矩形形状模拟）"""
    # 添加背景矩形
    ...
    background.fill.fore_color.rgb = RGBColor(25, 25, 35)  # 深色背景
...
def add_emoji_text(slide, emoji, text, top, font_size=28):
    ...
    run.font.size = Pt(font_size)
    run.font.color.rgb = RGBColor(255, 215, 0)  # 金色
    run.font.name = "PingFang SC"
    
    return textbox

def create_caixukun_ppt():
    """创建蔡徐坤介绍PPT"""
    prs = Presentation()
    ...
    create_gradient_slide(slide1, "封面")
    ...
    create_gradient_slide(slide2, "简介")
    ...
    create_gradient_slide(slide3, "出道")
    ...
    create_gradient_slide(slide4, "代表作")
    
    add_title_text(slide4, "🎵 代表作 & 名场面", Inches(0.3), font_size=40)
    
    # 左侧：音乐作品
    left_box = slide4.shapes.add_textbox(Inches(0.5), Inches(1.2), Inches(5.5), Inches(3))
    ...
    # 右侧：名场面
    right_box = slide4.shapes.add_textbox(Inches(6.5), Inches(1.2), Inches(6), Inches(5))
    ...
    add_emoji_text(slide4, "🐔", "「只因你太美~」—— 互联网永远的神", Inches(6.5), font_size=18)
    
    # ========== 第5页：总结 ==========
    slide5 = prs.slides.add_slide(blank_layout)
    create_gradient_slide(slide5, "总结")
    
    add_title_text(slide5, "📝 一句话总结", Inches(1), font_size=40)
    
    add_title_text(slide5, 
        "「他是一个被鬼畜成就的偶像，\n也是一个用作品证明自己的艺人」",
    ...
    add_bullet_points(slide5, summary_points, Inches(4), left=Inches(2), font_size=24)
    
    add_emoji_text(slide5, "🎤", "谢谢大家！记得给我点赞~", Inches(6.5), font_size=24)
    
    # 保存PPT
    output_path = "/Users/lillianliao/notion_rag/蔡徐坤介绍_轻松幽默版.pptx"
