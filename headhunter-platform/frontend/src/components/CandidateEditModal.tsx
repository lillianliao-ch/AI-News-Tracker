...
export default function CandidateEditModal({
  ...
  const fetchCandidateDetails = async () => {
    try {
      ...
      const newFormData = {
        ...
        workPermit: candidate.personalInfo?.workPermit || '',
        workYears: candidate.experience || '',
        education: candidate.education || '',
        linkedinUrl: candidate.personalInfo?.linkedIn || '',
        personalWebsite: candidate.personalInfo?.portfolio || ''
      }
      setFormData(newFormData)
      
      // 填充工作经历 - 从workHistory转换
      if (candidate.workHistory && candidate.workHistory.length > 0) {
        const workExps = candidate.workHistory.map((work: any) => ({
      ...
      if (candidate.educationHistory && candidate.educationHistory.length > 0) {
        const eduExps = candidate.educationHistory.map((edu: any) => ({
          school: edu.school || '',
          degree: edu.degree || '',
          major: edu.major || '',
        ...
        setEducationExperiences(eduExps)
      }
      
      // 填充项目经历 - 注意字段名是 projectExperience
      if (candidate.projectExperience && candidate.projectExperience.length > 0) {
        const projExps = candidate.projectExperience.map((proj: any) => ({
        ...
        setProjectExperiences(projExps)
      }
      
      // 培训经历 - API中没有此数据，保持默认空数组
      // if (candidate.trainingHistory && candidate.trainingHistory.length > 0) {
      //   const trainExps = candidate.trainingHistory.map((train: any) => ({
      //     name: train.name || '',
      ...
      //   }))
      //   setTrainingExperiences(trainExps)
      // }
      
      // 填充文件信息
      if (candidate.attachments) {
        setFiles(candidate.attachments)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onCandidateUpdated(response.candidate || response)
      onClose()
      
    } catch (err: any) {
      setError(err.message || '更新候选人失败')
      console.error(err)
    } finally {
      setLoading(false)
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-orange-500 text-white px-6 py-4 flex justify-between items-center">
          <h2 className="text-xl font-medium">编辑候选人信息</h2>
          <button
            onClick={onClose}
