...
// Schema定义
const createHunterPostSchema = z.object({
  type: z.enum(['job_seeking', 'talent_recommendation']),
  title: z.string().min(1).max(100),
  content: z.string().min(1).max(1000)
})

const getHunterPostsSchema = z.object({
  type: z.enum(['all', 'job_seeking', 'talent_recommendation']).default('all'),
...
// 权限检查函数
function getUserPermissions(userStatus: string) {
  switch (userStatus) {
    case 'guest':
...
// 猎头信息分享路由
export async function hunterPostsRoutes(fastify: FastifyInstance) {
  ...
  fastify.patch('/:id/view', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = request.params as { id: string }
