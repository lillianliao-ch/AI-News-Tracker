...
exports.hunterAuthRoutes = hunterAuthRoutes;
...
// Schema定义
const quickRegisterSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(50),
    phone: zod_1.z.string().min(10).max(20),
    wechatId: zod_1.z.string().optional(),
    reason: zod_1.z.string().min(10).max(500).optional()
});
...
// 生成JWT token
function generateToken(userId, role, status) {
    return jsonwebtoken_1.default.sign({ id: userId, role, status }, JWT_SECRET, { expiresIn: '7d' });
}
// 验证JWT token
...
// 猎头用户认证路由
async function hunterAuthRoutes(fastify) {
    ...
    fastify.get('/stats', {
        ...
        try {
            const [totalUsers, registeredUsers, verifiedUsers, inactiveUsers, todayRegistrations] = await Promise.all([
                prisma.user.count({
...
//# sourceMappingURL=hunter-auth.js.map